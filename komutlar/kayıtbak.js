...
exports.run = async (bot, message, args) => {
  if (!message.member.roles.cache.find(x=>x.id==="YONETICI ROL ID"))
  return message.channel.send(
    'Bu komudu kullacak kadar yetkili değilsin üzgünüm.'
  );
  let yetkili = message.mentions.members.first();
  if(!yetkili) return message.reply("Birini etiketlemedin")
